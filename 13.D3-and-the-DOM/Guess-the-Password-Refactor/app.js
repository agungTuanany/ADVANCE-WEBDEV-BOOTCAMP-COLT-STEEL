document.addEventListener('DOMContentLoaded', () => {
  ...
  // in general where ever you see 'doucment.getElementById' that opportnity to use d3.select()
  ...
  function startGame() {
    // get randon words and append them to the DOM
  ...
  function shuffle(array) {
    ...
    for (var idx1 = arrayCopy.length - 1; idx1 > 0; idx1 --) {
      // geenerate random index between 0 and idx1 (inclusive)
      var idx2 = Math.floor(Math.random() * (idx1 + 1));

      // swap elements at idx1 and idx2
      var temp = arrayCopy[idx2];
  ...
  function setGuessCount(newCount) {
    guessCount = newCount;
    d3.select('guess-remaining')
      .text("Guessess remaining: " + guessCount + ".");
  }
